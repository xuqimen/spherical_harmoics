...
void computeP_real_vector_v2( const size_t L ,
	const double * const A , const double * const B ,
	double * const P , double *x, const int len)
{
#define P(m,n) P[(m)+(n)*len]
// #define P(m,n) P[(m)*tot_len+(n)]
	double *sintheta = malloc(len * sizeof(*sintheta));
	double *temp = malloc(len * sizeof(*temp));
	assert(sintheta != NULL && temp != NULL);
	// = sqrt (1/ 4 M_PI )
	int tot_len = PT(L,L)+1;
	int PT_00 = PT(0, 0);
	int PT_10 = PT(1, 0);
	int PT_11 = PT(1, 1);
	const double SQRT1DIV4PI = 0.282094791773878; // sqrt(1/ (4 M_PI))
	// for (int i = 0; i < len; i++){
	// 	temp[i] = 0.282094791773878 ; // = sqrt (1/ (4 M_PI) )
	// 	P[i*tot_len + PT_00 ] = 0.282094791773878 ; 
	// 	sintheta[i] = sqrt(1.0 - x[i]*x[i]) ;
	// }
	for (int i = 0; i < len; i++) {
	...
	if (L > 0) {
		const double SQRT3 = 1.7320508075688772935 ;
		const double SQRT3DIV2 = -1.2247448713915890491;
		const double SQRT3DIV4PI = (SQRT3 * SQRT1DIV4PI);
		const double mSQRT3DIV8PI = (SQRT3DIV2 * SQRT1DIV4PI);
		for (int i = 0; i < len; i++) {
			P(i,PT_10) = x[i] * SQRT3DIV4PI;
			double temp_val = sintheta[i] * mSQRT3DIV8PI;
			temp[i] = temp_val;
			P(i,PT_11) = temp_val;
		}
		...
		for ( size_t l = 2; l <= L ; l ++) {
			...
			for ( size_t m = 0; m < l-1; m ++) {
				...
				int PT_lm2 = PT(l-2, m);
				for (int i = 0; i < len; i++){
					P(i, PT_lm) = A[PT_lm]*(x[i]*P(i, PT_lm1)
					+ B[PT_lm]* P(i, PT_lm2)) ;
				}
			}
			for (int i = 0; i < len; i++){
				P(i,PT_ll1) = x[i]*temp_l*temp[i];
				temp[i] = - temp_l2 * sintheta[i] * temp[i] ;
				P(i,PT_ll) = temp[i];
			}
		}
...
void computeY_real_vector_v2( const size_t L , const double * const P ,
	double * const Y , double *phi, const int len ) {
#define Y(m,n) Y[(m)+(n)*len]
#define P(m,n) P[(m)+(n)*len]
// #define Y(m,n) Y[(m)*total_lengthY+(n)]
// #define P(m,n) P[(m)*total_lengthP+(n)]
	int total_lengthY = 1+YR(L,L);
	int total_lengthP = 1+PT(L,L);
	for (size_t l = 0; l <= L ; l ++){
		int YR_l0 = YR(l, 0);
	...
	for (size_t m = 1; m <= L ; m ++) {
		const int m_sign = 1 - 2 * (m%2); // m is even: 1, m is odd: -1
		for (int i=0; i<len; i++){
			s[i] = tc[i] * s1[i] - s2[i];
			c[i] = tc[i] * c1[i] - c2[i];
			s2[i] = s1[i]; s1[i] = s[i];
		...
		for ( size_t l = m ; l <= L ; l ++) {
			...
			for (int i=0; i<len; i++){
				// if (m%2==0){
				// 	Y(i,YR_lm2) = P(i,PT_lm) * s[i]*SQRT2 ;
				// 	Y(i,YR_lm ) = P(i,PT_lm) * c[i]*SQRT2;
				// } else {
				// 	Y(i,YR_lm2) = -P(i,PT_lm) * s[i]*SQRT2 ;
				// 	Y(i,YR_lm ) = -P(i,PT_lm) * c[i]*SQRT2;
				// }
				double temp_val = m_sign * P(i,PT_lm) * SQRT2;
				Y(i,YR_lm2) = temp_val * s[i];
				Y(i,YR_lm ) = temp_val * c[i];
			}
		}
	}
#undef Y
...
void sph_harmonics_real_vector_v2(const int len, double *theta, double *phi, const int LL,
	...
	computeP_real_vector_v2( LL , &A[0] , &B[0] , P , x, len);
	computeY_real_vector_v2( LL , P , Y ,  phi, len );
